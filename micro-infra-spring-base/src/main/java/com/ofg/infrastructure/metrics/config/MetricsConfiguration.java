...
public class MetricsConfiguration {
    ...
    public Graphite graphite(@Value("${graphite.host:graphite.4finance.net}") String hostname, @Value("${graphite.port:2003}") int port) {
        return new Graphite(new InetSocketAddress(hostname, port));
    }

    @Bean(initMethod = "start", destroyMethod = "stop")
    @Profile(BasicProfiles.PRODUCTION)
    public GraphitePublisher graphitePublisher(Graphite graphite, MetricRegistry metricRegistry, @Value("${graphite.publishing.interval:15000}") long publishingIntervalInMs) {
    ...
    @Bean
    public MetricPathProvider metricPathProvider(@Value("${metrics.path.root:apps}") String rootName, @Value("${metrics.path.environment:test}") String environment, @Value("${metrics.path.country:pl}") String country, @Value("${metrics.path.app:service-name}") String appName) {
